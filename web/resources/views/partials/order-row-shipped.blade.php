...
<tr class="border-b border-slate-100 hover:bg-slate-50/50">
    ...
    <td class="px-4 py-2 align-middle text-xs" data-label="Order">
        <div class="flex flex-col gap-0.5">
            ...
            --}}
            <span class="text-slate-600 font-normal" :title="order.business_central ? ('BC sales order: ' + (order.business_central?.number || '')) : 'Demo placeholder. No live Business Central order is linked.'" x-text="order.business_central ? ('BC: ' + (order.business_central?.number || '')) : 'BC nr:(placeholder)'"></span>
            <span class="text-slate-600 font-normal" :title="order.webshipper ? (order.webshipper?.tracking_numbers?.length > 0 ? 'Tracking: ' + (order.webshipper.tracking_numbers.join(', ')) : 'Webshipper order #' + order.webshipper?.order_id) : 'Demo placeholder. No live Webshipper order is linked.'" x-text="order.webshipper ? ('WS: ' + (order.webshipper?.status || ('#' + order.webshipper?.order_id)) + (order.webshipper?.tracking_numbers?.length > 0 ? ' (' + order.webshipper.tracking_numbers.length + ' tracking)' : '')) : 'WS nr:(placeholder)'"></span>
        </div>
    </td>
