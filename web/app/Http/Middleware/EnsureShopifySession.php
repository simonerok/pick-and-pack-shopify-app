...
class EnsureShopifySession
{
    ...
    public function handle(Request $request, Closure $next, string $accessMode = self::ACCESS_MODE_OFFLINE)
    {
        ...
        try {
            ...
            // Initial embedded page loads may not include Authorization header.
            // Treat as no active session and continue to re-auth flow below.
            $session = null;
        }
        ...
        $authUrl = '/api/auth' . (count($params) ? ('?' . http_build_query($params)) : '');
